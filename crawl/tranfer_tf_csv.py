...
def get_video(total_tf,total_write ):
    # The path to the TensorFlow record
    for i in range(len(total_tf)):

        video_lvl_record = total_tf[i]
...
def main():
    ...
    for i in range(len(total_tf)):
        thread = threading.Thread(target=get_video, args=(total_tf[i], total_write[i], ))
        thread.start()
        print('Thread done : ', i)
